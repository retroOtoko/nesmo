...
define(['chrScripts/utils.js'], function(utils) {
    ...
    sprite.prototype.get_color = function(index) {
        return palette[index];
    };

    sprite.prototype.load_sprites = function(file) {
        var chr = utils.open_file(file);
        var sprites = [];
    ...
    sprite.prototype.load_sprites_from_array = function(array) {
        var sprites = [];
        for (var i = 0; i < array.length; i++) {
            sprites.push(array[i] & 0xFF);
        }
        return sprites;
    };

    sprite.prototype.get_sprite = function(index, sprites) {
        var iA = index * 16;
        var iB = iA + 8;
        var iC = iB + 8;
        var channelA = sprites.slice(iA, iB);
        var channelB = sprites.slice(iB, iC);
        return this.decode_sprite(channelA, channelB);
    };

    sprite.prototype.decode_sprite = function(channelA, channelB) {
        var sprite = [];
        for (var y = 0; y < 8; y++) {
            var a = channelA[y];
            var b = channelB[y];
    ...
    sprite.prototype.put_sprite = function(index, sprites, spr) {
        var start = index * 16;
    ...
    sprite.prototype.encode_sprite = function(spr) {
        var channelA = [];
        var channelB = [];
        for (var y = 0; y < 8; y++) {
            var a = 0;
