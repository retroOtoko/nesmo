...
define(['chrScripts/sprite.js'], function(sprite) {
    ...
    Widget.prototype.right_side = function() {
        return this.position_x + this.width;
    };
    Widget.prototype.bottom = function() {
        return this.position_y + this.height;
    };
    ...
    Widget.prototype.onReset = function(widget) {
        this.sprites = widget.sprites;
        this.render();
    };
    ...
    Widget.prototype.notifyReset = function() {
        for (var l in this.redrawListeners) {
            this.redrawListeners[l].onReset(this);
        }
    };
    ...
    SpriteSelector.prototype.set_width = function(newWidth) {
        var canvasContext = this.canvas.getContext('2d');
        canvasContext.clearRect(this.position_x, this.position_y, this.width, this.height);

        var squareWidth = newWidth / this.sprite_x;
        this.sprite_id = 0;
        this.pixelSize = (squareWidth * (2 / 19));
        this.pixelPadding = Math.floor(squareWidth * (3 / 19));
        this.spriteSize = Math.floor(this.pixelSize * 8);
        this.page = 0;
        this.width = this.sprite_x * (this.spriteSize + this.pixelPadding) - this.pixelPadding;
        this.height = this.sprite_y * (this.spriteSize + this.pixelPadding) - this.pixelPadding;
        this.render();
    ...
    SpriteSelector.prototype.nextPage = function() {
        if (this.page < (this.sprites.length / (16 * this.sprite_x * this.sprite_y)) - 1) {
            this.page++;
        }
    };
    ...
    SpriteSelector.prototype.click = function(x, y) {
        if (this.previousPageButton !== undefined && x >= this.previousPageButton.position_x && x <= this.previousPageButton.position_x + this.previousPageButton.width && y >= this.previousPageButton.position_y && y <= this.previousPageButton.position_y + this.previousPageButton.height) {
            this.previousPage();
            this.render();
        } else if (this.nextPageButton !== undefined && x >= this.nextPageButton.position_x && x <= this.nextPageButton.position_x + this.nextPageButton.width && y >= this.nextPageButton.position_y && y <= this.nextPageButton.position_y + this.nextPageButton.height) {
            this.nextPage();
            this.render();
        } else if (x >= this.position_x && x <= this.position_x + this.width && y >= this.position_y && y <= this.position_y + this.height) {
            var line = Math.abs((this.position_y - y) / (this.spriteSize + this.pixelPadding) >> 0);
            var col = Math.abs((this.position_x - x) / (this.spriteSize + this.pixelPadding) >> 0);
    ...
    SpriteSelector.prototype.render = function() {
        ...
        if (this.previousPageButton) this.previousPageButton.context.drawImage(this.previousPageButton, this.previousPageButton.position_x, this.previousPageButton.position_y);
        if (this.nextPageButton) this.nextPageButton.context.drawImage(this.nextPageButton, this.nextPageButton.position_x, this.nextPageButton.position_y);
    };
    SpriteSelector.prototype.onColorChanged = function(widget) {
        this.palette = widget.palette;
        this.render();
    };
    SpriteSelector.prototype.onReset = function(widget) {
        this.page = 0;
        this.render();
    };
    ...
    SpriteSelector.prototype.addNextPageButton = function(img_src, x, y) {
        ...
        this.nextPageButton.onload = function() {
          if(this.position_x == 0) this.position_x = this.width; //Hack to make sure this displays to the right of the previousPageButton on first load.
            this.context.drawImage(this, this.position_x, this.position_y);
        };
    ...
    Palette.prototype.set_picker_size = function(newSize) {
        this.picker_size = newSize;
        this.width = this.picker_size * 4;
        this.height = this.picker_size;
        this.render();
    };
    Palette.prototype.render = function() {
        ...
        for (var x = 0; x < 4; x++) {
            context.beginPath();
            context.rect(x * this.picker_size + this.position_x, this.position_y, this.picker_size, this.picker_size);
        ...
        context.beginPath();
        context.rect(this.palette_id * this.picker_size + this.position_x, this.position_y, this.picker_size, this.picker_size);
        context.lineWidth = 2;
    ...
    var ColorPicker = function(canvas, position_x, position_y) {
        ...
        this.num_rows = 4;
        this.num_cols = 16;
        this.width = this.picker_size * this.num_cols;
        this.height = this.picker_size * this.num_rows;
        this.render();
    };
    ColorPicker.prototype = new Widget();
    ColorPicker.prototype.constructor = ColorPicker;
    ColorPicker.prototype.set_picker_size = function(newSize) {
        this.picker_size = newSize;
        this.width = this.picker_size * 16;
        this.height = this.picker_size * 4;
        this.render();
    };
    ColorPicker.prototype.recalculate_size = function() {
        this.width = this.picker_size * this.num_cols;
        this.height = this.picker_size * this.num_rows;
    }
    ColorPicker.prototype.render = function() {
        ...
        for (var y = 0; y < this.num_rows; y++) {
            for (var x = 0; x < this.num_cols; x++) {
                if (color_index > 63) break;
                context.beginPath();
                context.rect(x * this.picker_size + this.position_x, y * this.picker_size + this.position_y, this.picker_size, this.picker_size);
    ...
    ColorPicker.prototype.click = function(x, y) {
        var line = Math.abs((this.position_y - y) / this.picker_size >> 0);
        var col = Math.abs((this.position_x - x) / this.picker_size >> 0);
        if (line * col >= 63) return;

        this.color_id = line * this.num_cols + col;
        this.notifyColorChanged();
    };
    ...
    SpriteLoader.prototype.load_array = function(array) {
        this.sprites = sprite.load_sprites_from_array(array);
        this.file = undefined;
        this.notifyReset();
        this.notifyRedraw();
    };
    SpriteLoader.prototype.addUpdateCompileButton = function(img_src, x, y) {
        ...
        this.updateCompileButton.onload = function() {
          if(this.position_x == 0) this.position_x = this.width + this.width; //Hack to make sure this displays to the right of the nextPageButton on first load.
            this.context.drawImage(this, this.position_x, this.position_y);
        };
        this.updateCompileButton.src = img_src;
    };
    SpriteLoader.prototype.render = function() {
        if (this.updateCompileButton) this.updateCompileButton.context.drawImage(this.updateCompileButton, this.updateCompileButton.position_x, this.updateCompileButton.position_y);
    };
    SpriteLoader.prototype.was_clicked = function(x, y) {
        if (this.updateCompileButton !== undefined && x >= this.updateCompileButton.position_x && x <= this.updateCompileButton.position_x + this.updateCompileButton.width && y >= this.updateCompileButton.position_y && y <= this.updateCompileButton.position_y + this.updateCompileButton.height) {
            return true;
        }
    };
